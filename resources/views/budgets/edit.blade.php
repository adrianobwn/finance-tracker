            ...
            <form action="{{ route('budgets.update', $budget->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="amountDisplay" class="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0" value="{{ number_format($budget->amount, 0, ',', '.') }}" oninput="formatAmount(this)" required>
                        <input type="hidden" name="amount" id="amountValue" value="{{ $budget->amount }}" required>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="category_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <option value="{{ $cat->id }}" {{ $budget->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->icon }} {{ $cat->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ $budget->start_date->format('Y-m-d') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Selesai</label>
                        <input type="date" name="end_date" value="{{ $budget->end_date->format('Y-m-d') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                    </div>
                </div>
